...
INSERT INTO `grupo` VALUES (1,'11'),(2,'22'),(3,'33'),(4,'44');
...
/*!50001 VIEW `estudiantes_x_grupo` AS select `g`.`grupo_id` AS `grupo_id`,`g`.`grupo_nombre` AS `grupo_nombre`,`e`.`estudiante_id` AS `estudiante_id`,`e`.`estudiante_nrc` AS `estudiante_nrc`,`e`.`estudiante_apellidos` AS `estudiante_apellidos`,`e`.`estudiante_nombre` AS `estudiante_nombre` from (`estudiante` `e` left join `grupo` `g` on((`e`.`estudiante_grupo_id` = `g`.`grupo_id`))) order by `e`.`estudiante_grupo_id`,`e`.`estudiante_nrc`,`e`.`estudiante_id`,`e`.`estudiante_apellidos`,`e`.`estudiante_nombre` */;
...
-- Dump completed on 2019-04-24 18:18:06
